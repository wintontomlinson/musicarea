...
export default async function HomePage() {
  ...
  return <div className="app-page"><section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="section-kicker">MusicArea presents</p><h1 className="mt-2 text-h2 font-extrabold tracking-[-0.04em] sm:text-h1">{greeting()}, <span className="headline-gradient">let&apos;s move.</span></h1><p className="mt-2 max-w-xl text-[15px] text-text-secondary">Your selected languages set the scene. Choose a song and keep the queue going.</p></div><div className="tint-chip w-fit"><Icon name="disc" size={15} />Turn up your world</div></section>{hero && <Hero song={hero} />}{rows.map((row) => <Carousel key={`${row.id}-${row.title}`} row={row} />)}{browse?.moods && <MoodGrid moods={browse.moods} heading="Pick a mood and press play" />}</div>;
}
