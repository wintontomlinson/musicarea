...
export function Topbar() {
  ...
  return (
    // `--chrome-alpha` keeps the toolbar more transparent than the sidebar: it
    // sits directly over scrolling content, where the sidebar does not.
    <header className="chrome-panel sticky top-0 z-30 border-b border-white/10 px-4 py-3 [--chrome-alpha:0.65] sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-[1680px] items-center gap-3">
        ...
        <button type="button" aria-label="Queue" aria-pressed={queueOpen} onClick={() => setQueueOpen(!queueOpen)} className={`hidden h-9 w-9 shrink-0 place-items-center rounded-full transition lg:grid ${queueOpen ? 'bg-accent text-on-accent shadow-glow' : 'bg-white/[0.07] text-text-secondary hover:bg-white/[0.12] hover:text-white'}`}>
        ...
        <form role="search" onSubmit={submit} className="relative min-w-0 flex-1 lg:max-w-[250px] lg:flex-none">
          ...
          <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search music" aria-label="Search" className="w-full rounded-full border border-white/10 bg-white/[0.07] py-2 pl-9 pr-3 text-[13px] text-white outline-none transition placeholder:text-text-muted focus:border-accent/60 focus:bg-white/[0.12]" />
        </form>
...
function AvatarButton() {
  ...
  return <Avatar name={profile?.name || 'Listener'} avatarId={profile?.avatar ?? AVATARS[0].id} size={31} className="ring-2 ring-accent/40 shadow-glow" />;
}
