...
export function FullPlayer() {
  ...
  return (
    <section aria-label="Now playing" className="fixed inset-0 z-50 overflow-hidden bg-[#0d0b13]">
      <div className="absolute inset-0"><Image src={cover} alt="" fill priority sizes="100vw" className="scale-125 object-cover opacity-20 blur-3xl" /><div className="absolute inset-0 bg-gradient-to-b from-[#0d0b13]/45 via-[#0d0b13]/80 to-[#0d0b13]" /></div>
      <div ref={sheetRef} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onTouchCancel={onTouchEnd} className={`relative mx-auto flex h-full max-w-md flex-col overflow-y-auto ${settling ? 'transition-[transform,opacity] duration-300 ease-smooth' : ''}`}>
        <header className="flex items-center justify-between px-5 py-4"><button type="button" aria-label="Minimize player" onClick={() => setFullscreen(false)} className="grid h-10 w-10 place-items-center rounded-lg bg-white/[0.08] text-white"><Icon name="collapse" size={22} /></button><span className="text-[12px] font-medium text-white/70">Now playing</span><button type="button" aria-label="Show queue" onClick={() => setQueueOpen(true)} className="grid h-10 w-10 place-items-center rounded-lg bg-white/[0.08] text-white"><Icon name="queue" size={19} /></button></header>
        <div className="flex flex-1 flex-col justify-center gap-7 px-6 pb-10"><div className="relative aspect-square w-full overflow-hidden rounded-xl2 shadow-[0_28px_60px_-28px_rgba(0,0,0,.9)]"><Image src={cover} alt={track.name} fill priority sizes="448px" className="object-cover" /></div><div><h1 className="truncate text-h3 font-extrabold tracking-tight">{track.name}</h1><p className="mt-1 truncate text-h5 text-white/65">{artistLine(track)}</p></div><SeekBar showTimes /><TransportControls size="full" /></div>
      </div>
    </section>
  );
}
