...
export function FullPlayer() {
  ...
  function apply(dy: number) {
    ...
    if (!sheet) return;
    if (dy <= 0) { sheet.style.transform = ''; sheet.style.opacity = ''; return; }
  ...
  function onTouchMove(event: React.TouchEvent) {
    ...
    if (!state.decided) {
      if (raw < 6) return;
      if ((sheetRef.current?.scrollTop ?? 0) > 0) { state.active = false; return; }
      state.decided = true;
    }
    state.dy = Math.max(0, raw); apply(state.dy);
  }
  function onTouchEnd() {
    const state = drag.current;
    if (!state.active) return;
    ...
    setSettling(true);
    if (state.dy > 120 || (velocity > 0.6 && state.dy > 60)) setFullscreen(false); else apply(0);
  }
  return (
