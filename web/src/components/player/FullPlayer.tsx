...
export function FullPlayer() {
  ...
  return <section aria-label="Now playing" className="fixed inset-0 z-50 overflow-hidden bg-scrim"><div className="absolute inset-0"><Image src={cover} alt="" fill priority sizes="100vw" className="scale-150 object-cover opacity-30 blur-3xl" /><div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_10%,rgb(var(--accent-rgb)/.35),transparent_35%),radial-gradient(circle_at_90%_0%,rgb(var(--accent-alt-rgb)/.23),transparent_34%),linear-gradient(to_bottom,rgb(var(--scrim-rgb)/.5),rgb(var(--scrim-rgb)/.96))]" /></div><div ref={sheetRef} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onTouchCancel={onTouchEnd} className={`relative mx-auto flex h-full max-w-md flex-col overflow-y-auto ${settling ? 'transition-[transform,opacity] duration-300 ease-smooth' : ''}`}><header className="flex items-center justify-between px-5 py-4"><button type="button" aria-label="Minimize player" onClick={() => setFullscreen(false)} className="grid h-10 w-10 place-items-center rounded-full border border-white/15 bg-white/[0.08] text-white"><Icon name="collapse" size={22} /></button><span className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.15em] text-white/70"><span className="neon-dot" />Now playing</span><button type="button" aria-label="Show queue" onClick={() => setQueueOpen(true)} className="grid h-10 w-10 place-items-center rounded-full border border-white/15 bg-white/[0.08] text-white"><Icon name="queue" size={19} /></button></header><div className="flex flex-1 flex-col justify-center gap-7 px-6 pb-10"><div className="relative aspect-square w-full overflow-hidden rounded-xl2 border border-white/20 shadow-glow-lg"><Image src={cover} alt={track.name} fill priority sizes="448px" className="object-cover" /></div><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="section-kicker mb-1">On the dance floor</p><h1 className="truncate text-h3 font-extrabold tracking-tight">{track.name}</h1><p className="mt-1 truncate text-h5 text-white/65">{artistLine(track)}</p></div><LikeButton song={track} size={22} className="mt-6 h-10 w-10 shrink-0 rounded-full border border-white/15 bg-white/[0.08]" /></div><SeekBar showTimes /><TransportControls size="full" /></div></div></section>;
}
