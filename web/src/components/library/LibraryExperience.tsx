...
function CollectionLink({ href, icon, title, count, hydrated, empty }: { href: string; icon: IconName; title: string; count: number; hydrated: boolean; empty: string }) {
  return <Link href={href} className="surface-card flex items-center gap-4 p-4 transition hover:-translate-y-0.5 hover:border-accent/35 hover:shadow-glow"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-accent/30 bg-accent/10 text-accent-soft"><Icon name={icon} size={21} /></span><span className="min-w-0 flex-1"><span className="block text-[15px] font-bold">{title}</span><span className="mt-0.5 block text-[13px] text-text-secondary">{!hydrated ? 'Checking this device…' : count ? `${count} ${count === 1 ? 'track' : 'tracks'} saved here` : empty}</span></span><Icon name="chevronRight" size={16} className="shrink-0 text-text-muted" /></Link>;
}

function SessionEmpty() {
  return <section className="premium-panel flex flex-col items-center justify-center p-8 text-center sm:p-12"><span className="grid h-14 w-14 place-items-center rounded-full bg-brand text-on-accent shadow-glow"><Icon name="disc" size={25} /></span><h2 className="mt-4 text-h4 font-extrabold">The floor is open</h2><p className="mt-2 max-w-md text-[14px] leading-relaxed text-text-secondary">Start a song from Home, Explore, Charts or Search. Its active queue will appear here for this listening session.</p></section>;
}
