...
export function Hero({ song }: { song: Song }) {
  ...
  return (
    <section className="disco-panel min-h-[300px] sm:min-h-[360px]">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-[#160b2d]/95 via-[#160b2d]/80 to-[#130b28]/35" />
      <div className="absolute -right-14 -top-20 h-64 w-64 rounded-full border border-cyan-200/30 bg-cyan-300/10 blur-[1px]" />
      <div className="absolute -right-6 top-9 grid h-36 w-36 place-items-center rounded-full border-[18px] border-fuchsia-200/20 bg-black/20 shadow-glow"><span className="h-8 w-8 rounded-full bg-fuchsia-300/80" /></div>
      <div className="relative flex h-full flex-col justify-end gap-6 p-5 sm:flex-row sm:items-end sm:justify-between sm:gap-8 sm:p-8 lg:p-10">
        <div className="min-w-0 max-w-2xl">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3"><PlayPill song={song} /><span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-white/65"><Icon name="sparkle" size={14} className="text-cyan-200" />Fresh from the catalogue</span></div>
        </div>
